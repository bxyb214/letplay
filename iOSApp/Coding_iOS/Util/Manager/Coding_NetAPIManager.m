...
#pragma mark Project
- (void)request_Projects_WithObj0:(Projects *)projects andBlock:(void (^)(Projects *data, NSError *error))block{
    [MobClick event:kUmeng_Event_Request label:@"项目列表"];
    projects.isLoading = YES;
    [[CodingNetAPIClient sharedJsonClient] requestJsonDataWithPath:[projects toActPath] withParams:[projects toActParams] withMethodType:Get andBlock:^(id data, NSError *error) {
        projects.isLoading = NO;
        if (data) {
            //id resultData = [data valueForKeyPath:@"data"];
            Projects *pros = [NSObject objectOfClass:@"Projects" fromJSON:@{
            @"list": data
            }];
            block(pros, nil);
        }else{
...
// Let's Play.
-(void)request_NewInvitation_WithObj:(Invitation *)invitation image:(UIImage *)image andBlock:(void (^)(NSString *, NSError *))block{
    [MobClick event:kUmeng_Event_Request label:@"Send invitation"];
    [self showStatusBarQueryStr:@"Sending invitation"];
    ...
    [[CodingNetAPIClient sharedJsonClient] requestJsonDataWithPath:[invitation toCreatePath] file:nil withParams:[invitation toCreateParams] withMethodType:Post andBlock:^(id data, NSError *error) {
        if (data) {
            [self showStatusBarSuccessStr:@"Send invitation successully."];
            id resultData = [data valueForKeyPath:@"data"];
            block(resultData, nil);
        }else{
            [self showStatusBarError:error];
