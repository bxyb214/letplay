...
- (id)initWithFrame:(CGRect)frame projects:(Projects *)projects block:(ProjectListViewBlock)block  tabBarHeight:(CGFloat)tabBarHeight
{
    ...
    if (self) {
        ...
        _myTableView = ({
            ...
            //[tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"ICell"];
            //[tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:kCellIdentifier_ProjectListTaCell];
...
//        if (list.count > 0) {
            [_dataList addObject:@{kTitleKey : @"一般项目",
                                   kValueKey : self.myProjects.list}];
        //}
    //}
}
...
- (void)sendRequest{
    ...
    [[Coding_NetAPIManager sharedManager] request_Projects_WithObj0:_myProjects andBlock:^(Projects *data, NSError *error) {
   ...
   // if (_myProjects.type < ProjectsTypeTaProject) {
    ...
    if (cell == nil) {
        ...
        cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle
                                      ...
                                      reuseIdentifier:@"ICell"];

    }
    Invitation *invit= [[self valueForSection:indexPath.section] objectAtIndex:indexPath.row];
    cell.imageView.image = [UIImage imageNamed:@"coding_emoji_29.png"];
    cell.detailTextLabel.text = invit.description_mine;
    cell.textLabel.text = invit.title;
    return cell;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath00:(NSIndexPath *)indexPath{
    if (_myProjects.type < ProjectsTypeTaProject) {
